...
# First-time visitors go to quiz before home
if not st.session_state.quiz_done and st.session_state.page == "home":
...
def show_home():
    ...
    # Re-take quiz button at the bottom of the home page
    st.divider()
    _, center, _ = st.columns([2, 1, 2])
    with center:
        if st.button("🔍  Help me choose a type", use_container_width=True):
            st.session_state.quiz_done    = False
            st.session_state.quiz_step    = 0
            st.session_state.quiz_answers = {}
            st.session_state.quiz_result  = None
